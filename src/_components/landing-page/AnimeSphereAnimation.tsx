...
const AnimeSphereAnimation = () => {
  ...
  useEffect(() => {
    ...
    // Simple CSS animation fallback since anime.js v4 has different API
    ...
    const animateStrokes = () => {
      spherePathEls.forEach((path, index) => {
        ...
        setTimeout(() => {
          element.style.transition = 'stroke 2s ease-in-out, stroke-dashoffset 3s ease-in-out'
    ...
    // Breathing effect with color transitions
    const breathingInterval = setInterval(() => {
      spherePathEls.forEach((path, index) => {
        const element = path as SVGPathElement
        const phase = (animationIndex + index * 0.1) * 0.02
        const opacity = 0.3 + 0.7 * (Math.sin(phase) + 1) / 2
        
        element.style.stroke = `rgba(59, 130, 246, ${opacity})`
      })
      animationIndex++
    }, 50)
  ...
  return (
    <div className="flex h-full w-full items-center justify-center">
      <div className="relative w-full h-full flex items-center justify-center">
        <div ref={sphereRef} className="w-full h-full max-w-[300px] max-h-[300px]">
