...
const EmbededPanel: React.FC<RightPanelProps> = ({ isOpen, onClose }) => {
  ...
  // Use wagmi chainId directly - no need for chain switching in embedded panel
  ...
  // Fetch crosschain transfer history for external wallet sender
  const {
    ...
    queryFn: async () => {
      ...
      console.log(
        `[HISTORY] Fetching crosschain transfers for sender: ${externalWalletAddress}`
      );
    ...
    enabled:
      isOpen && !!externalWalletAddress && externalWalletAddress !== 'Not Connected', // Only fetch when panel is open and external wallet is connected
  ...
  // For embedded wallets, use the embedded wallet's actual chain ID
  // For external wallets or when no embedded wallet, use header's chain ID or core chain as fallback
  const balanceDisplayChainId = embeddedWalletChainId || chainId || getCoreChain();

  // Get token addresses for the balance display chain
  const getTokenAddressForBalanceChain = (contractName: ContractName) => {
    try {
      return balanceDisplayChainId
        ? getContractAddress(balanceDisplayChainId, contractName)
        : quoteCurrency;
    } catch (error) {
      console.warn(
        `No ${contractName} address for chain ${balanceDisplayChainId}, using fallback`
      );
  ...
  useEffect(() => {
    ...
    const tokenMap = new Map<
      string,
      { address: HexAddress; symbol: string; decimals: number; isQuote: boolean }
    >();
  ...
  // Source wallet uses appchain/arbitrum addresses, destination wallet (GTX) uses rari addresses
  const HARDCODED_TOKENS = [
    // USDT
    {
      ...
      sourceAddresses: {
        4661: '0x1362Dd75d8F1579a0Ebd62DF92d8F3852C3a7516' as HexAddress, // appchain
        421614: '0x5eafC52D170ff391D41FBA99A7e91b9c4D49929a' as HexAddress, // arbitrum sepolia
      },
    },
    // WETH
    {
      ...
      chainId: embeddedWalletChainId || balanceDisplayChainId,
      sourceAddresses: {
        4661: '0x02950119C4CCD1993f7938A55B8Ab8384C3CcE4F' as HexAddress, // appchain
        421614: '0x6B4C6C7521B3Ed61a9fA02E926b73D278B2a6ca7' as HexAddress, // arbitrum sepolia
      },
    },
    // WBTC
    {
      ...
      chainId: embeddedWalletChainId || balanceDisplayChainId,
      sourceAddresses: {
        4661: '0xb2e9Eabb827b78e2aC66bE17327603778D117d18' as HexAddress, // appchain
        421614: '0x24E55f604FF98a03B9493B53bA3ddEbD7d02733A' as HexAddress, // arbitrum sepolia
      },
    },
  ];

  // For crosschain: use hardcoded tokens OR pool tokens based on embedded wallet's actual chain
  // For non-crosschain: use only pool tokens
  const allUniqueTokens = FEATURE_FLAGS.CROSSCHAIN_DEPOSIT_ENABLED
    ? embeddedWalletChainId === 1918988905
      ? HARDCODED_TOKENS
      : uniqueTokens // Use hardcoded tokens only if embedded wallet is on Rari
    : uniqueTokens;
  ...
  // Use embedded wallet's actual chain ID for balance queries
  ...
  // Use sourceAddresses for crosschain tokens (source wallet), otherwise use regular address
  // IMPORTANT: Use chainId (from header) for external wallet balance queries to align with selected network
  const token0SourceAddress =
    (token0 as any)?.sourceAddresses?.[chainId] || token0?.address;
  ...
  // IMPORTANT: Use chainId (from header) for balance manager queries to align with selected network
  ...
  // IMPORTANT: Use chainId (from header) for external wallet balance queries to align with selected network
  const token1SourceAddress =
    (token1 as any)?.sourceAddresses?.[chainId] || token1?.address;
  ...
  // IMPORTANT: Use chainId (from header) for external wallet balance queries to align with selected network
  const token2SourceAddress =
    (token2 as any)?.sourceAddresses?.[chainId] || token2?.address;
  ...
  // IMPORTANT: Use chainId (from header) for external wallet balance queries to align with selected network
  const token3SourceAddress =
    (token3 as any)?.sourceAddresses?.[chainId] || token3?.address;
  ...
  // IMPORTANT: Use chainId (from header) for external wallet balance queries to align with selected network
  const token4SourceAddress =
    (token4 as any)?.sourceAddresses?.[chainId] || token4?.address;
  ...
  if (tokenBalances.length > 0) {
    assets = tokenBalances
      ...
      .map((tb, index) => {
        ...
        return {
          symbol: tb.symbol,
          balance: `${formatNumber(Number(tb.displayBalance), {
            decimals: 2,
            compact: true,
          })} ${tb.symbol}`,
  ...
  useEffect(() => {
    console.log('🔍 Token loading debug:', {
      ...
      uniqueTokensLength: uniqueTokens.length,
      uniqueTokens: uniqueTokens.map(t => ({
        symbol: t.symbol,
        address: t.address.slice(0, 8) + '...',
      })),
  ...
  // Set default selected tokens when tokens are loaded - use first available token
  useEffect(() => {
    if (tokens.length > 0) {
      // Always set the first token as default if none is selected or if the current selection is invalid
      const defaultToken = tokens[0];
      const currentTokenExists = tokens.find(
        t => t.address === selectedDepositTokenAddress
      );

      if (!selectedDepositTokenAddress || !currentTokenExists) {
        console.log('🚀 Setting default token (first available):', {
          defaultToken: defaultToken.address,
          symbol: defaultToken.symbol,
          tokensLength: tokens.length,
          previousSelection: selectedDepositTokenAddress,
  ...
  }, [tokens.length, selectedDepositTokenAddress, allUniqueTokens.length, tokens]);

  const currentDepositToken = tokens.find(
    token => token.address === selectedDepositTokenAddress
  );
  const currentWithdrawToken = tokens.find(
    token => token.address === selectedWithdrawTokenAddress
  );

  // Debug logging for current token state
  useEffect(() => {
    console.log('🎯 Current token state:', {
      selectedDepositTokenAddress,
      selectedDepositTokenSymbol: currentDepositToken?.symbol,
      currentDepositTokenAddress: currentDepositToken?.address,
      tokensAvailable: tokens.map(t => ({
        symbol: t.symbol,
        address: t.address.slice(0, 8) + '...',
      })),
      tokensLength: tokens.length,
      currentDepositTokenExists: !!currentDepositToken,
    });
  }, [selectedDepositTokenAddress, currentDepositToken, tokens]);
  ...
  const handlePrivyDeposit = async () => {
    ...
    console.log(
      '⚡ handlePrivyDeposit: CROSSCHAIN_DEPOSIT_ENABLED:',
    ...
    if (FEATURE_FLAGS.CROSSCHAIN_DEPOSIT_ENABLED) {
      ...
      // For crosschain deposits, use the chain selected in header as the source chain
      // This ensures the embedded panel aligns with the header's chain selection
      const sourceChainId = chainId || defaultChainId; // Chain selected in header
      const userExternalChainId = chainId;

      console.log(
        '⚡ handlePrivyDeposit: sourceChainId (header selected chain):',
        sourceChainId
      );
      console.log(
        '⚡ handlePrivyDeposit: userExternalChainId (header selected chain):',
        userExternalChainId
      );
      ...
      if (!isCrosschainSupportedChain(sourceChainId)) {
        console.log(
          '⚡ handlePrivyDeposit: Source chain not supported for crosschain deposits:',
          sourceChainId
        );
      ...
      // Check if external wallet needs to switch chains to match header selection
      if (externalWalletChainId && externalWalletChainId !== sourceChainId) {
        console.log(
          '⚡ handlePrivyDeposit: External wallet chain mismatch with header selection, attempting to switch'
        );
        console.log(
          '⚡ handlePrivyDeposit: From chain:',
        ...
        try {
          ...
          console.log(
            '⚡ handlePrivyDeposit: Requesting chain switch to:',
            sourceChainId
          );
          ...
          try {
            ...
            console.log(
              '⚡ handlePrivyDeposit: Chain switch failed, trying to add chain first:',
              switchError
            );
            ...
            if (
              ...
              const wagmiChain = wagmiConfig.chains.find(
                chain => chain.id === sourceChainId
              );

              if (!wagmiChain) {
                throw new Error(
                  `Chain configuration not found for chain ID ${sourceChainId}`
                );
              }
              ...
              console.log(
                '⚡ handlePrivyDeposit: Using RPC URL for chain',
                sourceChainId,
                ':',
                actualRpcUrl
              );
              ...
              console.log(
                '⚡ handlePrivyDeposit: Chain added successfully, now switching'
              );
          ...
          setToast({
            message:
              'Failed to switch chain. Please manually switch your external wallet to match the selected network in the header and try again.',
            type: 'error',
            loading: false,
            duration: 8000,
          });
      ...
      // For crosschain deposits, we need the SOURCE token address (appchain/arbitrum), not destination (rari)
      const selectedUniqueToken = allUniqueTokens.find(
        ut => ut.address === selectedToken?.address
      );
      const sourceTokenAddress =
        (selectedUniqueToken as any)?.sourceAddresses?.[chainId] ||
        selectedToken?.address;
      const tokenAddress =
        sourceTokenAddress ||
        uniqueTokens.find(t => t.isQuote)?.address ||
        uniqueTokens[0]?.address;

      console.log(
        '⚡ handlePrivyDeposit: Chain validation passed, calling crosschainDeposit hook'
      );
      ...
      console.log(
        '⚡ handlePrivyDeposit: Parameters - amount:',
        depositAmount,
        'token:',
        tokenAddress,
        'recipient:',
      ...
      try {
        ...
        console.log(
          '⚡ handlePrivyDeposit: crosschainDeposit function called successfully'
        );
      } catch (error) {
  ...
  const handlePrivyWithdraw = () => {
    setToast({
      message: `Withdrawing ${withdrawAmount} ${
        currentWithdrawToken?.symbol || 'token'
      }...`,
      type: 'info',
      loading: true,
      duration: 0,
  ...
  useEffect(() => {
    if (
      ...
      crosschainDepositCurrentStep === 'Crosschain deposit submitted successfully!'
    ) {
      depositResetState();
  ...
  useEffect(() => {
    if (!isOpen) {
      return; // Don't run interval when panel is closed
    }
      ...
      >
        ...
        <div className="p-4 border-b border-gray-700">
          ...
          <div className="flex items-center justify-between">
            ...
            {/* Display current network (read-only, controlled by header) */}
            <div className="text-xs text-gray-400">
              {(() => {
                const currentChainId = chainId || embeddedWalletChainId;
                const chainName =
                  currentChainId === 4661
                    ? 'Appchain'
                    : currentChainId === 421614
                    ? 'Arbitrum'
                    : currentChainId === 1918988905
                    ? 'Rari'
                      ...
                      >
                        ...
                        <div className="flex flex-col">
                          ...
                          {(() => {
                            ...
                            const selectedUniqueToken = allUniqueTokens.find(
                              ut => ut.address === currentDepositToken?.address
                            );
                            const sourceAddress =
                              (selectedUniqueToken as any)?.sourceAddresses?.[chainId] ||
                            ...
                            return (
                              sourceAddress && (
                                <span className="text-gray-400 text-xs font-mono">
                                  {`${sourceAddress.slice(0, 6)}...${sourceAddress.slice(
                                    -4
                                  )}`}
                                </span>
                              )
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-400">
                    ...
                    <span>
                      {(() => {
                        ...
                        return formatNumber(Number(sourceBalance || '0'), {
                          decimals: 2,
                          compact: true,
                        });
                      })()}
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="relative">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-white font-medium">
                            {currentDepositToken?.name
                              ? `gs${currentDepositToken.name}`
                              : ''}
                          </span>
                          {currentDepositToken?.address && (
                            <span className="text-gray-400 text-xs font-mono">
                              {`${currentDepositToken.address.slice(
                                0,
                                6
                              )}...${currentDepositToken.address.slice(-4)}`}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <span className="text-right font-medium w-full ml-4 text-gray-300">
                      {(() => {
                        ...
                        return formatNumber(Number(tokenBalance?.displayBalance || '0'), {
                          decimals: 2,
                          compact: true,
                        });
                      })()}
              ...
              >
                ...
                (crosschainDepositLoading &&
                  crosschainDepositCurrentStep !==
                    'Crosschain deposit submitted successfully!') ? (
                  <div className="flex items-center justify-center gap-2">
                    <RefreshCw className="w-4 h-4 animate-spin" />
                    <span>
                      {FEATURE_FLAGS.CROSSCHAIN_DEPOSIT_ENABLED
                        ? 'Processing Crosschain Deposit...'
                        : 'Depositing...'}
                    </span>
                  </div>
                ) : (
          ...
          {activeTab === 'History' && (
            <div className="p-4">
              <div className="mb-4">
                ...
                ) : !crosschainHistoryData?.crossChainTransferss?.items?.length ? (
                  <div className="text-center py-8">
                    <History className="w-12 h-12 text-gray-500 mx-auto mb-2" />
                    <p className="text-gray-400">No transaction history yet</p>
                    <p className="text-gray-500 text-sm mt-1">
                      Crosschain transactions initiated from your external wallet will
                      appear here
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {crosschainHistoryData.crossChainTransferss.items.map(
                      (transfer, index) => {
                        const isIncoming = transfer.direction === 'DEPOSIT';
                        const timestamp = new Date(
                          parseInt(transfer.dispatchMessage.timestamp) * 1000
                        );
                        ...
                        const getExplorerUrl = (
                          ...
                          const baseUrl =
                            urlsConfig.EXPLORER_URLS[
                              id as keyof typeof urlsConfig.EXPLORER_URLS
                            ];
                        ...
                        const getTokenInfo = (address: string) => {
                          ...
                          // USDT - 6 decimals (Appchain addresses)
                          if (
                          ...
                          // WETH - 18 decimals (Appchain and Arbitrum addresses)
                          else if (
                          ...
                          // USDC - 18 decimals (Appchain address) and 6 decimals (Arbitrum address)
                          else if (
                            addr === '0x02950119c4ccd1993f7938a55b8ab8384c3cce4f'
                          ) {
                            return { symbol: 'USDC', decimals: 18 };
                          } else if (
                            addr === '0x6fcf28b801c7116ca8b6460289e259ac8d9131f3'
                          ) {
                            return { symbol: 'USDC', decimals: 6 };
                          }
                          // WBTC - 8 decimals (Arbitrum addresses)
                          else if (
                            addr === '0x24e55f604ff98a03b9493b53ba3ddebd7d02733a'
                          ) {
                        ...
                        const amount =
                          parseFloat(transfer.amount) / Math.pow(10, tokenInfo.decimals);
                        const sourceChainName = getChainName(transfer.sourceChainId);
                        // For crosschain deposits, destination is always Rari (core chain)
                        const destChainName =
                          transfer.direction === 'DEPOSIT'
                            ? 'Rari'
                            : getChainName(transfer.dispatchMessage.chainId);
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-400 space-y-1">
                              ...
                              <div className="flex justify-between">
                                ...
                                <a
                                  href={getExplorerUrl(
                                    transfer.dispatchMessage.chainId,
                                    transfer.dispatchMessage.transactionHash
                                  )}
                                  target="_blank"
                                ...
                                >
                                  {transfer.dispatchMessage.transactionHash.slice(0, 8)}
                                  ...{transfer.dispatchMessage.transactionHash.slice(-6)}
                              ...
                              <div className="flex justify-between">
                                ...
                                >
                                  {transfer.dispatchMessage.messageId.slice(0, 8)}...
                                  {transfer.dispatchMessage.messageId.slice(-6)}
                                  <ExternalLink className="w-3 h-3" />
                                </a>
                              </div>
                            </div>
                            ...
                            <div className="mt-2 pt-2 border-t border-gray-700">
                              ...
                              {(() => {
                                // Check if there's a PROCESS message
                                const processMessage =
                                  transfer.dispatchMessage.pairedMessages?.items?.find(
                                    pairedMsg => pairedMsg.type === 'PROCESS'
                                  );

                                if (processMessage) {
                                  // Show completed destination transaction
                                  return (
                                    <div className="text-xs text-gray-400 flex justify-between items-center">
                                      <span>{getChainName(processMessage.chainId)}:</span>
                                      <a
                                        href={getExplorerUrl(
                                          processMessage.chainId,
                                          processMessage.transactionHash
                                        )}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="font-mono text-blue-400 hover:text-blue-300 flex items-center gap-1"
                                      ...
                                      >
                                        {processMessage.transactionHash.slice(0, 8)}...
                                        {processMessage.transactionHash.slice(-6)}
                                        <ExternalLink className="w-3 h-3" />
                                      </a>
                                    </div>
                                  );
