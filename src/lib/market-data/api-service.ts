 ...
 */
export async function fetchPoolsData(chainId: number = Number(DEFAULT_CHAIN)): Promise<PoolsPonderResponse | PoolsResponse> {
  try {
 ...
 */
export async function fetchTradesData(chainId: number = Number(DEFAULT_CHAIN)): Promise<TradesPonderResponse | TradesResponse> {
  try {
    const url = GTX_GRAPHQL_URL(chainId);
    if (!url) throw new Error("GraphQL URL not found");
 ...
 */
export async function fetchAndProcessMarketData(chainId: number = Number(DEFAULT_CHAIN)): Promise<MarketData[]> {
  try {
