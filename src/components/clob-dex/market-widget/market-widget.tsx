...
export default function MarketDataWidget() {
  ...
  const { data, isLoading, error } = useQuery<TradesResponse>({
    ...
    refetchInterval: 5000, // Changed from 500ms to 5000ms for better performance
    staleTime: 0,
  })
  ...
  useEffect(() => {
    if (data?.tradess?.items && data.tradess.items.length > 0) {
      ...
      const sortedItems = [...data.tradess.items].sort(
      ...
      if (sortedItems.length > 0) {
        ...
        // Get trading pair from the first trade's pool
        const pair = currentTrade.pool?.coin || 'ETH-USDC'
      ...
      <div className="flex items-center space-x-2 w-72">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <span className="font-medium text-lg">{marketData.pair || 'ETH-USDC'}</span>
